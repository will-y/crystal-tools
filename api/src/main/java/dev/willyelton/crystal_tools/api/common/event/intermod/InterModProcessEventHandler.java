...
import dev.willyelton.crystal_tools.api.common.levelable.condition.LevelableCondition;
import dev.willyelton.crystal_tools.api.common.levelable.condition.LevelableConditions;
...
public class InterModProcessEventHandler {
    ...
    private static void handleLevelableConditions(Stream<InterModComms.IMCMessage> stream) {
        stream.forEach(message -> {
            CrystalToolsApi.LOGGER.debug("Processing levelable condition for {}", message.senderModId());
            CrystalToolsApi.LOGGER.trace(message);
            ...
            if (condition instanceof LevelableCondition levelableCondition) {
                ...
                CrystalToolsApi.LOGGER.warn("Unable to process levelable condition for {}. Object was not a LevelableCondition", message.senderModId());
            }
        });
    }
    ...
    private static void handleSkillTreeBlacklists(Stream<InterModComms.IMCMessage> stream) {
        stream.forEach(message -> {
            CrystalToolsApi.LOGGER.debug("Processing skill tree blacklist for {}", message.senderModId());
            CrystalToolsApi.LOGGER.trace(message);
