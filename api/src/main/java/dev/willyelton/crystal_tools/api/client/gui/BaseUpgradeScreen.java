...
import dev.willyelton.crystal_tools.api.client.gui.component.SkillButton;
import dev.willyelton.crystal_tools.api.client.gui.component.XpButton;
import dev.willyelton.crystal_tools.api.client.gui.config.CrystalToolsCoreClientConfig;
...
import dev.willyelton.crystal_tools.api.common.skill.requirement.NodeOrSkillDataRequirement;
import dev.willyelton.crystal_tools.api.common.skill.requirement.NodeSkillDataRequirement;
import dev.willyelton.crystal_tools.api.common.skill.requirement.NotNodeSkillDataRequirement;
import dev.willyelton.crystal_tools.api.common.skill.requirement.RequirementType;
import dev.willyelton.crystal_tools.api.common.skill.requirement.SkillDataNodeRequirement;
import dev.willyelton.crystal_tools.api.common.skill.requirement.SkillDataRequirement;
import dev.willyelton.crystal_tools.api.common.skill.requirement.SkillItemRequirement;
import dev.willyelton.crystal_tools.api.utils.Colors;
import dev.willyelton.crystal_tools.api.utils.InventoryUtils;
import dev.willyelton.crystal_tools.api.utils.ListUtils;
import dev.willyelton.crystal_tools.api.utils.XpUtils;
...
public abstract class BaseUpgradeScreen extends Screen {
    ...
    /**
     * Return a string that is the location of a vanilla block texture,
     * or an {@link Identifier} for a block texture
     * @return a string for the background block texture
     */
    protected String backgroundLocation() {
        return CrystalToolsCoreClientConfig.UPGRADE_SCREEN_BACKGROUND.get();
    }
    ...
    protected int getPointsToSpend(int points, boolean shiftDown, boolean controlDown) {
        if (controlDown && shiftDown) {
            return Math.min(points, CrystalToolsCoreClientConfig.CONTROL_POINT_SPEND.get() * CrystalToolsCoreClientConfig.SHIFT_POINT_SPEND.get());
        } else if (controlDown) {
            return Math.min(points, CrystalToolsCoreClientConfig.CONTROL_POINT_SPEND.get());
        } else if (shiftDown) {
            return Math.min(points, CrystalToolsCoreClientConfig.SHIFT_POINT_SPEND.get());
        } else {
            return 1;
        }
    }
