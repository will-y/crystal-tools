...
public final class AttributeNode extends SkillDataNode implements ItemStackNode {
    ...
    public static final Map<Attribute, DeferredHolder<DataComponentType<?>, DataComponentType<Boolean>>> TOGGLEABLE_ATTRIBUTES = Map.of(
            ...
            dev.willyelton.crystal.core.common.datacomponent.DataComponents.CREATIVE_FLIGHT);
    ...
    public void processNode(SkillData skillData, ItemStack stack, int pointsToSpend, RegistryAccess registryAccess) {
        if (threshold) {
            int totalPoints = stack.getOrDefault(dev.willyelton.crystal.core.common.datacomponent.DataComponents.SKILL_POINT_DATA, new SkillPoints()).getPoints(this.getId());
        ...
        for (Identifier key : getKeys()) {
            Identifier rl = Identifier.fromNamespaceAndPath(ApiConstants.CORE_MOD_ID, key.getPath());
