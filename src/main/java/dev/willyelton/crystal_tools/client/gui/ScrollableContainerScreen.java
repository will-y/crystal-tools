...
public abstract class ScrollableContainerScreen<T extends AbstractContainerMenu & ScrollableMenu> extends AbstractContainerScreen<T> {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        if (this.canScroll()) {
            ...
            // Draw top
            guiGraphics.blit(RenderType::guiTextured, TEXTURE, scrollXActual, topPos, 0, 0, SCROLL_WIDTH + 7, scrollY, 256, 256);

            int heightDrawn = 1;

            // Top Pixel
            guiGraphics.blit(RenderType::guiTextured, TEXTURE, scrollXActual, scrollYActual, SCROLL_WIDTH + 7, 0, SCROLL_WIDTH + 7, 1, 256, 256);

            while (heightDrawn < scrollHeight - 254) {
                guiGraphics.blit(RenderType::guiTextured, TEXTURE, scrollXActual, scrollYActual + heightDrawn, SCROLL_WIDTH + 7, 1, SCROLL_WIDTH + 7, 254, 256, 256);
                heightDrawn += 254;
            }

            int remainingHeight = scrollHeight - heightDrawn;

            guiGraphics.blit(RenderType::guiTextured, TEXTURE, scrollXActual, scrollYActual + heightDrawn, SCROLL_WIDTH + 7, 256 - remainingHeight, SCROLL_WIDTH + 7, remainingHeight, 256, 256);

            // Draw bottom
            guiGraphics.blit(RenderType::guiTextured, TEXTURE, scrollXActual, scrollYActual + scrollHeight, 0, 251, SCROLL_WIDTH + 7, 5, 256, 256);

            // Draw handle
            guiGraphics.blit(RenderType::guiTextured, TEXTURE, scrollXActual + 1, scrollYActual + 1 + (int) ((float) (scrollYEnd - scrollYActual - 17) * scrollOffset), (SCROLL_WIDTH + 7) * 2, 0, HANDLE_WIDTH, HANDLE_HEIGHT, 256, 256);
        }
    }
