...
public class ModeSwitchHandler {
    ...
    public void handle(final ModeSwitchPayload payload, final IPayloadContext context) {
        ...
        if (tool.getOrDefault(DataComponents.MINE_MODE, false)) {
            if (payload.hasShiftDown() && !payload.hasCtrlDown() && !payload.hasAltDown()) {
                ...
                if (EnchantmentUtils.hasEnchantment(tool, Enchantments.SILK_TOUCH) && tool.getOrDefault(dev.willyelton.crystal.core.common.datacomponent.DataComponents.FORTUNE_BONUS, 0) > 0) {
                    EnchantmentUtils.removeEnchantment(tool, Enchantments.SILK_TOUCH);
                    EnchantmentUtils.addEnchantment(tool, Enchantments.FORTUNE, tool.getOrDefault(dev.willyelton.crystal.core.common.datacomponent.DataComponents.FORTUNE_BONUS, 3), player);
                    player.sendOverlayMessage(Component.literal("Mine Mode: Fortune"));
                } else if (EnchantmentUtils.hasEnchantment(tool, Enchantments.FORTUNE) && tool.getOrDefault(dev.willyelton.crystal.core.common.datacomponent.DataComponents.SILK_TOUCH_BONUS, false)) {
    ...
    private void disableCreativeFlight(Player player, ItemStack stack, SkillData skillData) {
        if (stack.getOrDefault(dev.willyelton.crystal.core.common.datacomponent.DataComponents.CREATIVE_FLIGHT, false)) {
