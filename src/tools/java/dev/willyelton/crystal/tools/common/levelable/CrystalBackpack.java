...
import dev.willyelton.crystal.tools.common.compat.curios.CuriosCompatibility;
...
public class CrystalBackpack extends Item implements LevelableItem {
    public CrystalBackpack(Properties properties) {
    ...
    @Override
    public void appendHoverText(ItemStack itemStack, TooltipContext context, TooltipDisplay display, Consumer<Component> components, TooltipFlag flag) {
    ...
    public static void addXpToBackpacks(Player player, int exp) {
        findBackpackStacks(player)
                .forEach(stack -> {
                    Levelable levelable = stack.getCapability(dev.willyelton.crystal.core.common.capability.Capabilities.ITEM_SKILL, player.level().registryAccess());
                    if (levelable != null) {
